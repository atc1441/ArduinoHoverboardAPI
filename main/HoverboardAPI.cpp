...
float HoverboardAPI::getCurrentLimit() {
  return electrical_measurements.dcCurLim;
}

int HoverboardAPI::getIsCharging() {
  return electrical_measurements.charging;
}

float HoverboardAPI::getTempInC() {
...
void HoverboardAPI::sendPowerOff(char powerOff, char som) {
  // Compose new Message
  PROTOCOL_MSG2 msg = {
    .SOM = som,
  };
  ...
  PROTOCOL_BYTES_WRITEVALS *writevals = (PROTOCOL_BYTES_WRITEVALS *) &(msg.bytes);
  char *writeenable = (char *) writevals->content;
